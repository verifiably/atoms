...
| `docs/certification/2026-08-28-ext4-linux-7.1.10-arch1-1.json` | historical/superseded | Valid zero-violation nine-scenario certification record superseded in the singleton allowlist by the 7.1.11 tuple. |
| `docs/certification/2026-08-30-ext4-linux-7.1.11-arch1-1.json` | authority/current | Zero-violation nine-scenario record named by the singleton `CERTIFIED_ALLOWLIST`; its recorded source commit is an ancestor and the schema/correspondence tests pass. |
...
| Current singleton allowlist/record correspondence, source-commit ancestry, exact document coverage, duplicate check, seven-section count, and `git diff --check` | Commit `914acb66e796f8691b7cc10bb7c28daa54dddfbf` makes the 7.1.11 record the singleton `CERTIFIED_ALLOWLIST` target; the record has nine scenarios and zero violations, its source commit is an ancestor, all 40 denominator documents are classified exactly once, and the section, coverage, duplicate, and whitespace checks pass. | `914acb66e796f8691b7cc10bb7c28daa54dddfbf` plus this ledger correction |
