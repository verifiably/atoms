...
| 12 | A3's `RecoveryPlan` orders transaction/effect transitions, the terminal rollback result, and structured halt diagnostics but performs no durable metadata I/O | A3 plan contract | A5 | Persist each A3 transition and diagnostic in plan order with the §7 SQLite durability barriers; persist `RESTORED` or `EXTERNAL_DRIFT_PRESERVED` atomically with `ROLLED_BACK`; never advance or detach metadata for a semantic step that A7 has not completed durably | §7.4, §13.1, §13.4 |
| 13 | A3 classifies opaque identity equality, file prefix relation, and modeled-versus-unmodeled directory occupancy but does not observe a filesystem | A3 observation contract | A6, A7 | Produce each entry's state, identity, prefix relation, and occupancy evidence coherently from held descriptors and the planned blob/resolved topology; never assemble one logical observation from separately re-resolved names or pre-classify it into a recovery verdict | §10, §13.1–§13.2 |
| 14 | A3 emits semantic tuple transformations and authorizes them against fresh evidence but names no syscall or durability sequence | A3 plan contract | A7 | Execute only factory-authorized steps, using the variant's §9 primitive and durability ordering; on fresh-evidence mismatch persist A3's halt instead of reclassifying, retrying, or inventing a second decision table | §9, §13.1–§13.2 |
