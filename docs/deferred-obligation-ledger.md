...
| 12 | A3's `RecoveryPlan` orders transaction/effect transitions, the separate commit decision, terminal rollback result, and frozen halt diagnostics but performs no durable metadata I/O | A3 plan contract | A5 | Persist each A3 transition and diagnostic in plan order with the §7 SQLite durability barriers; persist `RESTORED` or `EXTERNAL_DRIFT_PRESERVED` atomically with `ROLLED_BACK`; preserve `committed` and freeze the pre-halt state in the first halt diagnostic; never advance or detach metadata for a semantic step that A7 has not completed durably | §7.4, §8.1, §13.1, §13.4 |
| 13 | A3 classifies regular-file/directory identity equality, file prefix relation, symlink fingerprints, and modeled-versus-unmodeled directory occupancy but does not observe a filesystem | A3 observation contract | A6, A7 | Produce file/directory state and identity, prefix relation, and occupancy evidence coherently from held descriptors and the planned blob/resolved topology; produce symlink state only through the weaker `lstat` + `readlink` fingerprint and never manufacture descriptor identity for it; never assemble one logical observation from separately re-resolved names or pre-classify it into a recovery verdict | §6, §10, §13.1–§13.2 |
| 14 | A3 emits semantic tuple transformations and authorizes them against fresh evidence but names no syscall or durability sequence | A3 plan contract | A7 | Execute only factory-authorized mutating steps after every earlier step completed durably, using the variant's §9 primitive and durability ordering; on fresh-evidence mismatch persist A3's prefix-bound halt instead of reclassifying, retrying, or inventing a second decision table | §9, §13.1–§13.2 |
| 15 | A3's pure reducer proves semantic convergence but cannot prove that a real backend, SQLite ordering, or process restart implements the same transitions | A3 reducer | A8 | Run real-filesystem, subprocess, and persistence-cut cases against A3 decisions; require the executor's observed terminal states and second-pass behavior to match A3's fixed points | §13.2, §13.4 |
